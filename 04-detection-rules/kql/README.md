...
Current status: the SOC-2026-002 authentication query is **KQL_RESULT_RETAINED** from Azure Data Explorer / Kusto execution against the verified synthetic fixture. For SOC-2026-004, `unusual-admin-logon.kql`, `suspicious-powershell.kql` and `registry-run-key-persistence.kql` are **KQL_RESULT_RETAINED** from Azure Data Explorer / Kusto execution against verified synthetic inputs. The admin query used verified synthetic `Security004` and `AssetContext004`; the PowerShell and registry queries used verified synthetic `Sysmon004`. Python tests remain restricted local models, not KQL syntax/backend validation.
...
| [004 PowerShell](SOC-2026-004/suspicious-powershell.kql) | Custom Sysmon004 process table per query comments; retained ADX/Kusto runtime evidence | Projected event, host, user, image, parent and command fields; retained result `SYS-005` for encoded Office-child process | Authorized automation; narrow strings miss variants; synthetic fixture and ADX/Kusto execution do not prove malicious intent or production efficacy |
...
| [004 Run value](SOC-2026-004/registry-run-key-persistence.kql) | Sysmon004 registry table; retained ADX/Kusto runtime evidence | Projected target/value and process context; retained result `SYS-007` for suspicious Run configuration | Installers and updaters; synthetic fixture and ADX/Kusto execution do not prove malicious intent, effective write access or later persistence execution |
